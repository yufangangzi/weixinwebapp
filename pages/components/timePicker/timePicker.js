Component({
  ...
  methods: {
    ...
    initPick: function (initData) {
      ...
      const date = initData.initEndTime ? new Date(initData.initEndTime.replace(/-/g, '/')) : new Date();
      const nowYear = date.getFullYear();
      const nowMonth = date.getMonth() + 1;
      const nowDay = date.getDate();//this.data.rockbock ? date.getDate() - 1 : date.getDate() + 1;
      const nowHour = date.getHours();
      const nowMinute = date.getMinutes();
      const nowSecond = date.getSeconds();

      // const startDate = new Date(date.getTime() - 1000 * 60 * 60 * 24);
      const startDate = initData.initStartTime ? new Date(initData.initStartTime.replace(/-/g, '/')) : new Date();
      const startYear = startDate.getFullYear();
      const startMonth = startDate.getMonth() + 1;
      const startDay = startDate.getDate();//this.data.rockbock ? startDate.getDate() - 1 : startDate.getDate() + 1;
    ...
    setStartDate: function(year, month, day, hour, minute, second) {
      let pickerDateArr = this.setPickerDateArr(
        ...
        year,
        month,
        this.data.rockbock ? day - 1 + 1 : day,
        hour,
        minute,
        second
      );
